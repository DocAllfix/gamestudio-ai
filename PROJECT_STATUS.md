...
| 011 | `supabase/migrations/011_asset_enrichment.sql` | ⏳ pending apply | FASE 1 (Studio enrichment), Priorità 1. Adds `tile_size int`, `frame_meta jsonb`, `pixel_art boolean`, `enriched_at timestamptz` to `asset_library_index` + partial index `idx_assets_unenriched` for the incremental backfill. Palette reuses existing `image_color_palette`. Populated by the Studio detectors (lib/studio/{tile-size,palette,frame-analyzer}.ts). Additive, idempotent. Committed before apply. |
