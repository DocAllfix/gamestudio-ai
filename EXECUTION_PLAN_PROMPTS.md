...
- **`[dipende da v0.1.0-contracts]`**: marca i riferimenti che esistono solo dopo
  FASE 0 (babylon, `webExport()`, porte generative).
...
**Prompt operativo**:
```
Applica i contract proposal G.1, G.2, G.3 di docs/EXECUTION_ARCHITECTURE.md Parte G.
NON modificare i contratti esistenti oltre a queste aggiunte additive.
...
- [ ] `npx tsc --noEmit` → 0 errori
- [ ] ogni mock chiama `Schema.parse()` su input e output (verifica a vista)
...
**Output di fase**: `lib/orchestrator/hermes.ts` (sostituto reale di `orchestrator.mock.ts`) + `lib/reasoning/execution.ts`.

---

### [4-W1] — D.6 Evaluation Agent (smoke gate)

**Dipendenze**: `[3-W1]`. Transizione `runtime.mock` → reale avviene al **merge W3**
(prima di W1 nel merge order W2→W3→W1→W4).
...
game_exported_itch, fork), aggiorna il success_score:
...
- [ ] login Clerk funzionante (manuale) + webhook crea row `users`
...
- [ ] PWA: lighthouse "installable" pass (o manifest+SW presenti e validi)
- [ ] `npx tsc --noEmit` + `next build` → verdi
